...
import { EMAIL_REGEXP, ERROR_MESSAGES, PASSWORD_REGEXP } from "@/app/utiles/consts" 
...
export default function Register() {
	...
	function handleNewUser (event,target,err=null) {
		setNewUser({
			...newUser,
			[target]: {
	...
	const addUserOnCollection = async (user) => {
	...
	const handleImageUpload = async (file,user) => {
		const storageRef = ref(storage,`/users/${user.uid}.jpg`)
		try {
			const snapshot = await uploadBytes(storageRef, file)
			return newUser.image.value ? await getDownloadURL(storageRef) : await getDownloadURL(ref(storage,'/users/default_avatar.jpg'))				
		} 
		catch (error) {
	...
	const toRegister = async () => {
		setLoading(true)
		try {
			const user = (await createUserWithEmailAndPassword(auth,newUser.email.value,newUser.password.value)).user
